...
The Department of Public Health is no longer producing the daily PDF
report on Saturday or Sunday. Also the format for reporting nursing home
cases and deaths has changed, and I have not yet modified my code to
process the new format. So the nursing home data is not up to date.
...
[rt.live](https://rt.live/) for an estimate by state of the effective
...
of the calculation of R at the [rt.live](https://rt.live/) site, but it
...
`r format(max(rt_live$date),"%m/%d/%Y")` R<sub>t</sub> is
`r round(rt_live$mean[(rt_live$date == max(rt_live$date)) & (rt_live$region == "CT")], 2)`.
On the other hand, in Arizona R<sub>t</sub> is about
`r round(rt_live$mean[(rt_live$date == max(rt_live$date)) & (rt_live$region == "AZ")], 2)`.
...
80% confidence interval. Note that this map shows only whether
...
-   [rt.live](https://rt.live) I have made heavy use of this site in
    Figure \@ref(fig:p-rt-map) so I'll link to it here as well.
