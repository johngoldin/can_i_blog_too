...
tags: 
  ...
  - maps
  - census
description: 'Exploraton of geotagging and gender identification based on voter registration dataset for Guilford. Explores geotagging, identifing gender from first name, US Census, and general data clean-up. '
keywords: []
---
...
[site](http://connvoters.com/download.html) that collects a lot of voter registration data. 
One surprise is that it includes birth date and the full dataset also includes phone numbers. 
I was a bit shocked that this data is publicly available. 
Apparently I am not the only one. [Here](http://www.wfsb.com/story/26835982/website-displaying-voters-personal-information-has-right-to-do-so)
is an article from Eyewitness News in Connecticut treating this data as a controversy.
...
The author of this site is a fellow from new Hampshire. In his [about page](http://connvoters.com/about.html), he offers the following comment:
...
it something like a cross-stitch sampler that
I inherited from my mother. The point of samplers like this was just to practice the craft.
...
found in 2013, 2014, 2016, 2017, and 2018. Thqt's how he includes people who have moved away or died.
...
by party and sex. Women are less likely to be unaffiliated and more likely to be
...
ggplot(data = age2, aes(x = age, y = n, fill = sex)) +
...
ggplot(data = age, aes(x = age, y = n, fill = sex)) +
  geom_bar(stat = "identity", width = 1) + 
...
ggplot(data = age, aes(x = age, y = pct, fill = sex)) +
  geom_bar(stat = "identity", width = 1) + 
...
had to divde up the 8,000+ addresses into batches and feed them to google in bits and pieces
...
can see clearly how Google locates inidividual homes fairly well while the Digital Science Toolkit
