...
packages by [Kyle Walker](https://walkerke.github.io). The emphasis
...
33 topics](https://www.census.gov/ac/www/about/why-we-ask-each-question/) on the survey and why they are included.
...
(There's also a [new data exploration platform](https://data.census.gov/cedsci/results/tables?q=population%20of%20new%20haven&g=0500000US09009&ps=table*currentPage@1).)
...
race_town <- get_acs(geography = "county subdivision",  # for CT, that means towns
              state = "CT",
              county = "New Haven",
              geometry = "FALSE", 
              year = 2017,
...
race_town = race_town %>% left_join(rcodes, by = "variable") %>% 
  select(-NAME) %>% 
...
race_table <- as_tibble(race_town %>% 
                          select(TOWN, code, summary_est, pct, pct_moe, race))

race_table <- race_table %>% 
  mutate(pct = percent(pct, accuracy = 1.0)) %>% 
  select(Town = TOWN, race, Percent = pct) %>% 
...
and [tigris](https://github.com/walkerke/tigris) 
packages by [Kyle Walker](https://walkerke.github.io).
Several years I did a wee bit of work on making maps with census data without
