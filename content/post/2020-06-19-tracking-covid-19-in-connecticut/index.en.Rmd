...
```
.br
```{r p-rt-live, echo = FALSE, fig.asp = 0.3, fig.cap = "Estimated R"}
print(p_rt_live)
```
.br
...
```
.br
```{r p_hospitalizations, echo = FALSE, fig.cap = "Hospitalizations for Covid-19"}
...
```{r p-rt-map, echo = FALSE, fig.cap = "Estimate of R by State"}
print(p_rt_map)
```

### Variations by County in Connecticut

```{r county_trends, echo = FALSE}
...
```{r county-maps, echo = FALSE, fig.cap = "County Map of New Cases"}
print(p_county_map_new_cases)
```
```{r county-maps-cum-cases, echo = FALSE, fig.cap = "County Map Cumulative Cases"}
